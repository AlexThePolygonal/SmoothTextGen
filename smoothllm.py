...
def total_grad(model: torch.nn.Module, start_dim: int = 0) -> torch.Tensor:
    ...
    return torch.cat(
        [
            torch.flatten(p.grad, start_dim=start_dim)
...
class GradMod(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        temp = ctx.hook(None)  # the output is passed for debugging purposes
        return grad_output + temp, None


# Adds the gradient injection operation to a module
...
def bound_entropy(
    ...
    with torch.no_grad():
        excess_entropy_mask = (
            ...
            shape
        )  # batch elems with excessive entropy, of shape [..., 1]
...
class SmoothGenerationOutput:
    def __init__(
        self,
        model: torch.nn.Module,  # the base model. Needed here for backprop, as we re-generate the output on the backward pass
        toks: torch.LongTensor,  # Tokens generated by the model, of shape [batch_size, seq_len,topk]
        tokprobs: torch.Tensor,  # probabilities of the generated tokens, of shape [batch_size, seq_len, topk]
        kv_cache: Union[
            None, torch.Tensor
        ],  # the cache accumulated during this generation
        after_eos_mask: torch.BoolTensor,  # mask removes every token which is preceded by an <eos> token, of shape [batch_size, seq_len]
        saved_states: List[
            Tuple
        ],  # states of random generators and other temporary outputs
        config: SmoothGenerationConfig,
        generation_start_idx: int,  # where the generation started
    ):
        self.model = model
        self.toks = toks
...
class SmoothLoss:
    ...
    class LossValue:
        ...
        def backwards(self, do_clip_norms:bool = True, clip_norm:Optional[float] = 10., output=None):
            ...
            """
            rand_state = save_random_state(self.model_output.toks.device)
            ...
            load_random_state(rand_state, toks.device)
    ...
    for i in tqdm(range(iters)):
        ...
        for j in range(log_iters + 1):
            ...
            if i % k + 1 == k:
                ...
                csim = (
                    torch.cosine_similarity(jth_batch_storage[j], mean, dim=0)
                    .to("cpu")
                    .numpy()
                )
